...
document.getElementById("reset-button").addEventListener('click', () => {
    location.reload();  // Refreshes the page to reset the game
  });

  function changeTurnIndicator() {
    // Check if it's white's turn and update the turn indicator accordingly
    if (isWhiteTurn) {
...
function basicAI() {
    let allLegalMoves = getAllLegalMovesForBlack();
    let bestMove = null;
  
    // Prioritize capturing moves
    let captureMoves = allLegalMoves.filter(move => move.isCapture);
    if (captureMoves.length > 0) {
      bestMove = captureMoves[Math.floor(Math.random() * captureMoves.length)];
    } else {
      // If no captures, prefer advancement moves
      let advancementMoves = allLegalMoves.filter(move => move.isAdvancement);
      if (advancementMoves.length > 0) {
        bestMove = advancementMoves[Math.floor(Math.random() * advancementMoves.length)];
      } else {
  ...
  function getAllLegalMovesForBlack() {
    let legalMoves = [];
    
    boardSquaresArray.forEach(square => {
      if (square.pieceColor === 'black') {
        let piece = { pieceColor: square.pieceColor, pieceType: square.pieceType, pieceId: square.pieceId };
        let moves = getPossibleMoves(square.squareId, piece, boardSquaresArray);
  
        moves.forEach(move => {
          let destinationSquare = getPieceAtSquare(move, boardSquaresArray);
          let isCapture = destinationSquare.pieceColor === 'white';
          let isAdvancement = (square.squareId[1] > move[1]); // Moving forward
  ...
  function executeAIMove(move) {
    let piece = document.getElementById(move.piece.pieceId);
    let destinationSquare = document.getElementById(move.to);
  
    // Remove the captured piece if it's a capture move
    if (move.isCapture) {
      let capturedPiece = document.querySelector(`#${move.to} .piece`);
      if (capturedPiece) {
        capturedPiece.remove();
      }
    }
  
    // Move the AI piece to the destination square
    destinationSquare.appendChild(piece);
  
    // Update the board state
    updateBoardSquaresArray(move.from, move.to, boardSquaresArray);
